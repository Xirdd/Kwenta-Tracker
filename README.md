...
**Confirmed via live testing: Supabase requires aal2 (a completed 2FA challenge) to unenroll a factor — being signed in normally isn't enough on its own.** This affected two flows, fixed differently since they're genuinely different situations:

- **Profile → Disable 2FA** — fixed. Since someone using this still has their working authenticator, disabling now asks for one fresh code first, which completes the aal2 challenge before attempting to unenroll. This also doubles as a sensible safeguard on its own: proving you still have your authenticator before removing it.
- **The recovery-code path (lost authenticator) — still broken, and can't be fixed the same way.** By definition, someone using a recovery code doesn't have a working authenticator to get a fresh code from, so there's no way to reach aal2 client-side in that scenario. The client SDK has no way around this — removing a factor without aal2 needs the service-role key, same constraint as [true account deletion](#delete-account): it has to happen server-side, via an Edge Function using the admin API, not through anything the browser can call directly. **This means the emergency "lost your authenticator" recovery flow doesn't currently work** — worth fixing before trusting recovery codes as an actual safety net, via the same Edge Function pattern already set up for account deletion (`supabase/functions/delete-account/`).

**Confirmed via live testing: Supabase requires aal2 to unenroll a factor, no exceptions.** This affected both flows that remove a TOTP factor:

- **Disabling from Profile** — fixed by requiring a fresh authenticator code (`renderDisableCodeStep` in `mfaSetupSheet.js`) before attempting removal. `challengeAndVerifyTotp()` brings the session to aal2, then `unenrollTotp()` is allowed. This is a reasonable requirement anyway, not just a technical hoop — proving you still have the authenticator before removing it is exactly the kind of check you'd want here.
- **The recovery-code path** — this one couldn't be fixed the same way, since the entire point of a recovery code is that the authenticator is unavailable; requiring a fresh code would defeat its purpose. Fixed instead by having `verify_mfa_recovery_code()` in `schema.sql` delete directly from `auth.mfa_factors` at the database level, bypassing Supabase's client-side aal2 check entirely. This is deliberate, not a workaround to feel uneasy about: the recovery code itself is the proof of identity in this flow, the same way GitHub and Google's recovery codes work — using one disables 2FA outright rather than trying to fake a session upgrade that genuinely can't happen without the lost authenticator.
