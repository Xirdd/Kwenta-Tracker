...
function render() {
  ...
  `;
  app.insertAdjacentHTML('beforeend', `<button class="fab" id="fabBtn">+</button>`);
  attachEvents();
}

function attachEvents() {
  document.getElementById('prevMonth').onclick = () => { shiftMonth(-1); render(); };
  document.getElementById('nextMonth').onclick = () => { shiftMonth(1); render(); };
...
(function init() {
  ...
  document.getElementById('app').innerHTML = `<div style="padding:60px 10px;text-align:center;color:#8fa093;font-family:Inter,sans-serif;font-size:13px;">Opening the ledger…</div>`;
  initData();
  render();
})();
