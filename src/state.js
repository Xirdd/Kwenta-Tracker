...
export function monthTx(type) {
  return DATA.transactions.filter((t) => t.type === type && t.date && t.date.startsWith(state.monthKey));
}

export function totals() {
  const salary = Number(DATA.salary[state.monthKey]) || 0;
  const extraIncome = monthTx('income').reduce((s, t) => s + Number(t.amount || 0), 0);
  const totalIncome = salary + extraIncome;
  const totalExpense = monthTx('expense').reduce((s, t) => s + Number(t.amount || 0), 0);
  return { salary, extraIncome, totalIncome, totalExpense, balance: totalIncome - totalExpense };
}
