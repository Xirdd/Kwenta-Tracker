...
export function renderBudgets() {
  ...
  const spentByCat = {};
  exp.forEach((e) => { spentByCat[e.category] = (spentByCat[e.category] || 0) + Number(e.amount || 0); });
  const totalBudget = CATEGORIES.reduce((s, c) => s + (Number(DATA.budgets[c.id]) || 0), 0);
...
export function attachBudgetEvents() {
  document.querySelectorAll('.budgetInput').forEach((inp) => {
    inp.oninput = (e) => {
      ...
      DATA.budgets[cat] = e.target.value === '' ? undefined : Number(e.target.value);
      saveData();
      const spent = monthTx('expense').filter((x) => x.category === cat).reduce((s, x) => s + Number(x.amount || 0), 0);
