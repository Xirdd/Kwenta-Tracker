import { state, DATA, monthTx, monthLabel, totals, saveData } from '../state.js';
import { incCatInfo } from '../categories.js';
import { fmt, formatDate, escapeHtml } from '../format.js';

export function renderIncome() {
  const extras = monthTx('income').sort((a, b) => (b.date || '').localeCompare(a.date || ''));
  return `
...
export function attachIncomeEvents() {
  ...
  salaryInput.oninput = (e) => {
    DATA.salary[state.monthKey] = e.target.value === '' ? undefined : Number(e.target.value);
    saveData();
    const t = totals();
    document.querySelector('.balance-amount').textContent = (t.balance < 0 ? '-' : '') + fmt(Math.abs(t.balance));
    document.querySelectorAll('.mini-stat .amt')[0].textContent = fmt(t.totalIncome);
    document.querySelectorAll('.mini-stat .amt')[1].textContent = fmt(t.totalExpense);
  };
}
