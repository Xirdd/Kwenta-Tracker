...
export function openForm(type, tx) {
  ...
  const heading = tx ? `Edit ${type === 'expense' ? 'expense' : 'income'}` : `Add ${type === 'expense' ? 'expense' : 'income'}`;
  ...
  document.querySelectorAll('#catGrid .cat-opt').forEach((el) => {
    el.onclick = () => {
      ...
      document.querySelectorAll('#catGrid .cat-opt').forEach((o) => o.classList.remove('selected'));
  ...
  document.getElementById('saveBtn').onclick = () => {
    ...
    if (tx) {
      tx.desc = desc; tx.amount = amount; tx.category = chosenCat; tx.date = date;
    } else {
      DATA.transactions.push({ id: uid(), type, desc, amount, category: chosenCat, date });
    }
    saveData();
    closeSheet();
    onChange();
...
function flashField(id) {
  ...
  setTimeout(() => { el.style.borderColor = 'transparent'; }, 700);
}
