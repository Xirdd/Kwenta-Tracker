...
export function exportCSV() {
  ...
  DATA.transactions
    ...
    .forEach((tx) => {
      const label = tx.type === 'expense' ? catInfo(tx.category).label : incCatInfo(tx.category).label;
      rows.push([tx.date || '', tx.type === 'expense' ? 'Expense' : 'Income', label, tx.desc || '', tx.amount]);
    });
  ...
  a.href = url;
  a.download = 'kwenta_export_' + new Date().toISOString().slice(0, 10) + '.csv';
